...
@Slf4j
public class DesktopAndApiAppMain extends BisqExecutable {
    private BisqApp application;

    public DesktopAndApiAppMain() {
    ...
    */
    public static void main(String[] args) throws Exception {
        if (BisqExecutable.setupInitialOptionParser(args)) {
            ...
            Thread.currentThread().setContextClassLoader(DesktopAndApiAppMain.class.getClassLoader());

            new DesktopAndApiAppMain().execute(args);
        }
    }
    ...
    protected void launchApplication() {
        BisqApp.setAppLaunchedHandler(application -> {
            DesktopAndApiAppMain.this.application = (BisqApp) application;

            // Necessary to do the setup at this point to prevent Bouncy Castle errors
            CommonSetup.setup(DesktopAndApiAppMain.this.application);
            // Map to user thread!
    ...
    protected AppModule getModule() {
        return new DesktopAndApiModule(bisqEnvironment);
    }
